...
def test_retry_failed_scheduled_task_run(client, monkeypatch):
    from app.models.scheduled_task_run import ScheduledTaskRunStatus

    token = _register_and_login(client, "schedule-retry@example.com")
    workspace = _create_workspace(client, token, "Retry")
    headers = {"Authorization": f"Bearer {token}"}
    future = (datetime.now(timezone.utc) + timedelta(hours=2)).isoformat()
    ...
    # Create a failed run directly so the test does not depend on the AI provider.
    ...
    with next(override()) as db:
        run = ScheduledTaskRun(
            ...
            user_id=int(task["id"]) + 1000,
            workspace_id=workspace["id"],
            prompt="فشل",
            status=ScheduledTaskRunStatus.failed,
            error="خطأ تجريبي",
        )
        # Use the actual owner id through the task lookup.
        from app.models.scheduled_task import ScheduledTask
        db_task = db.get(ScheduledTask, task["id"])
        run.user_id = db_task.user_id
        db.add(run)
        db.commit()
        db.refresh(run)
        run_id = run.id
    ...
    response = client.post(
        f"/scheduled-tasks/{task['id']}/runs/{run_id}/retry",
        headers=headers,
    )
    assert response.status_code == 202
    retried = response.json()
    assert retried["scheduled_task_id"] == task["id"]

    runs = client.get(f"/scheduled-tasks/{task['id']}/runs", headers=headers)
    assert runs.status_code == 200
    assert len(runs.json()) == 2
    assert any(run["id"] == run_id and run["status"] == "failed" for run in runs.json())
