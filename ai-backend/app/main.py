...
configure_logging(); logger = get_logger("main")
...
async def security_headers_middleware(request: Request, call_next):
    ...
    if settings.ENVIRONMENT == "production": response.headers["Strict-Transport-Security"] = "max-age=63072000; includeSubDomains"
    return response
...
def health_check(db: Session = Depends(get_db)):
    try: db.execute(text("SELECT 1")); db_status = "ok"
        ...
        logger.exception("فحص الصحة: قاعدة البيانات غير متاحة"); db_status = "error"
    return {"status": "ok" if db_status == "ok" else "degraded", "database": db_status, "environment": settings.ENVIRONMENT, "app": settings.APP_NAME}

app.include_router(auth_router); app.include_router(two_factor_router); app.include_router(users_router); app.include_router(chat_router); app.include_router(conversations_router); app.include_router(files_router); app.include_router(billing_router); app.include_router(notifications_router); app.include_router(admin_router)
