...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column("paypal_plan_id", sa.String(length=100), nullable=True),
    ...
    op.create_table(
        ...
        sa.Column(
            "status",
            sa.Enum(
                "incomplete", "trialing", "active", "past_due", "canceled", name="subscriptionstatus"
            ),
            nullable=False,
        ),
    ...
    # خطتان افتراضيتان — عدّل stripe_price_id/paypal_plan_id لاحقًا من لوحة الإدارة أو مباشرة بقاعدة البيانات
    # لما تنشئ الأسعار الحقيقية بحساب Stripe/PayPal الخاص فيك
    plans_table = sa.table(
        ...
        sa.column("daily_ai_request_limit", sa.Integer),
